...
jQuery(document).ready(function ($) {
    ...
    //新建角色按钮
    $("#btnCompanyRole").live("click", function () {
        $('#DivAddCompanyRole').modal();
         LoadCompanyList();
    });
    ...
    $("#btnSaveAddRole").live("click", function () {
        ...
        var addHQRoleName = $("#addHQRoleName").val();

        //必填项检查

        if ($.checkEmpty("CompanyNameList", CompanyNameList, "该项为必填项!")) { return false; };
        if ($.checkEmpty("addHQRoleName", addHQRoleName, "该项为必填项!")) { return false; };
        var obj = {
            Url: "../../../BLL/HQ/Organization/CompanyPosition/frmCompanyPosition.ashx",
            Action: "addRole",
            FormId: "FormAddHQRole"
        };
        ...
        try {
            ...
            if (jsonText.Msg == "OK") {
                $.Alert("操作成功!");
                $('#DivAddCompanyRole').modal('hide');
                $.EmptyForm("FormAddHQRole");
                //刷新表单
                RefreshTable();
            }
            else {
    ...
    $("#BtnItemAuth").live("click", function (event) {
        ...
        var BranchCode = $(event.target).attr("BranchCode");
        var BranchName = $(event.target).attr("BranchName");


        $("#HideBranchCode").val(BranchCode);
        ...
        //动态加载优品端 功能列表
        LoadYPMenuList(RoleID, BranchCode);
        $('#DivAuthRole').modal()
    });

    //加载优品端菜单列表
    function LoadYPMenuList(RoleID,BranchCode) {


        var obj = {
            Url: "../../../BLL/HQ/Organization/CompanyPosition/frmCompanyPosition.ashx",
            Action: "LoadYPMenuList",
            Data: { "RoleID": RoleID, "BranchCode": BranchCode }
        };
        //提交  
        $.SubmitPostData(obj);
    ...
    $('#table').bootstrapTable({
        ...
        url: "../../../BLL/HQ/Organization/CompanyPosition/frmCompanyPosition.ashx?Action=Query",
           ...
           },
        {
            ...
            formatter: function (value, row, index) {
                ...
                if (row.CreateUser != "system") {
                    ...
                    Auth = '<a href="#"  id="BtnItemAuth"  SID="' + row.ID + '" BranchCode="' + row.BranchCode + '"  BranchName="' + row.BranchName + '" RoleName="' + row.RoleName + '"      class="btn btn-sm glyphicon glyphicon-th-large">功能授权</a>';
